...
class NeuralNetwork(object):
	...
	def fit(self, X, y, X_test, y_test, epochs=10, learning_rate=0.2, learning_rate_decay = 0 , momentum = 0, verbose = 0):
		...
		for k in range(epochs):
			...
			result = np.argmax(y_predicted, axis=1).astype(int)
			resultTest = np.argmax(y_test, axis=1).astype(int)

	  		print("\nClassification report for classifier:\n\n%s\n"
	 		 % (metrics.classification_report(resultTest, result)))
		  	print("Confusion matrix:\n\n%s" % metrics.confusion_matrix(resultTest, result))
